...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'eventStart' | 'eventEnd'> & {
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...event,
      eventStart: event.eventStart?.toJSON() ?? null,
      eventEnd: event.eventEnd?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...restEvent,
      eventStart: restEvent.eventStart ? dayjs(restEvent.eventStart) : undefined,
      eventEnd: restEvent.eventEnd ? dayjs(restEvent.eventEnd) : undefined,
    };
  }
