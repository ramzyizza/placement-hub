...
import { EntityArrayResponseType, CalendarService } from '../service/calendar.service';
...
export class CalendarComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.calendars = this.refineData(dataFromBody);
  }

  protected refineData(data: ICalendar[]): ICalendar[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ICalendar[] | null): ICalendar[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.calendarService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
